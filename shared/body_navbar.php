		<nav class="body_navbar">
			<ul>
				<li><b>View Axle's:</b></li>
				<li><a href="/styleguide/">Grid</a></li>
				<li><a href="/styleguide/color_generic.php">Color chart</a></li>
				<li><a href="/styleguide/color.php">Semantic colors</a></li>
				<li><a href="/styleguide/text.php">Text</a></li>
				<li><a href="/styleguide/form.php">Forms</a></li>
				<li><a href="/styleguide/buttons.php">Buttons</a></li>
				<li><a href="/styleguide/alerts.php">Alerts</a></li>
			</ul>
		</nav>